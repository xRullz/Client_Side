...
defender1.src = "/game/img/def1.png";
class Defender {
  constructor(x, y) {
    this.x = x;
    this.y = y;
    ...
    this.frameX = 0;
    this.frameY = 0;
    this.spriteWidth = 1421;
    this.spriteHeight = 1203;
    this.minFrame = 0;
    this.maxFrame = 12;
  }
  draw() {
...
function handleResources() {
  ...
  for (let i = 0; i < resources.length; i++) {
    resources[i].draw();
    if (resources[i] && mouse.x && mouse.y && collection(resources[i], mouse)) {
      numberOfResource += resources[i].amount;
      floatingMessages.push(
        new floatingMessage(
          "+" + resources[i].amount,
          resources[i].x,
...
function handleGameStatus() {
  ...
  ctx.fillText("Resources : " + numberOfResource, 180, 80);
  if (gameOver) {
    ctx.fillStyle = "black";
